...
void ConfigManager::Save(const json& config) {
	try {
		...
		Log("[ConfigManager::Save] JSON parse error: %s\n", e.what());
		return;
	}
	catch (...) {
		Log("[ConfigManager::Save] Unknown exception\n");
		return;
	}
}
...
json ConfigManager::Load() {
	try {
		...
		Log("[ConfigManager::Load] JSON parse error: %s\n", e.what());
		return json{};
	}
	catch (...) {
		Log("[ConfigManager::Load] Unknown exception\n");
		return json{};
	}
}
